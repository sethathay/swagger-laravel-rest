...
class UdcValueFoundationController extends Controller
{
    private $me;
    private $data;
    private $schema;
    private $schemaTable = "TUDCVALUEFOUNDATION";
    private $awsCognito;
    ...
    public function __construct(Request $request){
        ...
        $this->me = new UdcValueFoundation(Route::current()->parameter('sysCode'),
                                           Route::current()->parameter('typeId'));
        $this->schema = new SchemaRender($this->schemaTable);
        $this->awsCognito = new CognitoAws();
    }
    /**
     * Save udc value of foundation to database
     * @date 17-Jul-2017
     * @author Setha Thay <setha.thay@workevolve.com>
     * @param  string $sysCode
     * @param  string $typeId
     * @return newly created json of udc value foundation
     */
    public function create($sysCode,$typeId){
        try{
            ...
            if($validateSMS["status"]){
                $udcObj = $this->me->getUdc($sysCode, $typeId);
                if($udcObj){
                    $findObj = $udcObj->values()
                                    ->where(UdcValueFoundation::FIELD_RECORD_TYPE, $this->me->getRecordType())
                                    ->where(UdcValueFoundation::FIELD_SYS_CODE, $sysCode)
                                    ->where(UdcValueFoundation::FIELD_UDC, $udcObj->code)
                    ...
                    $udcValueObj = new UdcValueFoundation($sysCode, $typeId);
                    $ct->where(UdcValueFoundation::FIELD_RECORD_TYPE, $udcValueObj->getRecordType());
                    $ct->where(UdcValueFoundation::FIELD_SYS_CODE, $udcValueObj->getSysCode());
                    $ct->where(UdcValueFoundation::FIELD_UDC, $udcObj->code);
                    ...
                    $href = env('APP_URL') .env('APP_VERSION').'/systems/'. $sysCode . '/types/' . $typeId . '/values/'. $result->_id;
                    ...
                    $eventId = $revisionObj->newEvent(
                                            'Event - Saving New Udc Value Foundation', 
                                            $user
                                        );
     ...
     */
    public function update($sysCode,$typeId,$valueId){
        try{
            //Access Token Validation
            $user = $this->awsCognito->validateAccessToken();
            if(isset($this->data["id"])){ unset($this->data["id"]); }
            ...
            if($validateSMS["status"]){
                ...
                if($udcObj){
                    ...
                    $criteria->where(UdcValueFoundation::FIELD_RECORD_TYPE, $this->me->getRecordType());
                    $criteria->where(UdcValueFoundation::FIELD_SYS_CODE,$sysCode);
                    $criteria->where(UdcValueFoundation::FIELD_UDC, $udcObj->code);
                    ...
                    if($updateObj){
                        ...
                        $href = env('APP_URL') .env('APP_VERSION').'/systems/'. $sysCode . '/types/' . $typeId . '/values/'. $oldObj->_id;
                        ...
                        $href = env('APP_URL') .env('APP_VERSION').'/systems/'. $sysCode . '/types/' . $typeId . '/values/'. $outputObj->_id;
                        $result = $this->schema->renderOutput($outputObj,$href,$user);
                        //Saving audit trail and data versioning
                        $revisionObj = new RevisionManager();
                        $eventId = $revisionObj->newEvent(
                                                'Event - Modifying Udc Value Foundation', 
                                                $user
                                            );
                        $revisionObj->save($eventId,
                                            $this->schema->getAllFields(),
     ...
     * @return -
     */
    public function remove($sysCode,$typeId,$valueId){
        try{
            //Access Token Validation
            $user = $this->awsCognito->validateAccessToken();
            $udcObj = $this->me->getUdc($sysCode, $typeId);
            if($udcObj){
                //Getting type by sys code and id
                $criteria = new CriteriaOption();
                $criteria->where(UdcValueFoundation::FIELD_RECORD_TYPE, $this->me->getRecordType());
                $criteria->where(UdcValueFoundation::FIELD_SYS_CODE,$sysCode);
                $criteria->where(UdcValueFoundation::FIELD_UDC, $udcObj->code);
                ...
                if($targetObj){
                    ...
                    if(!$relationshipObj->inUsed($targetObj, $this->schema->getPluralName())){
                        $href = env('APP_URL') .env('APP_VERSION').'/systems/'. $sysCode . '/types/' . $typeId . '/values/'. $targetObj->_id;
                        ...
                        $revisionObj = new RevisionManager();
                        $eventId = $revisionObj->newEvent(
                                                'Event - Deleting Udc Value Foundation', 
                                                $user
                                            );
     ...
     */
    public function readAll($sysCode,$typeId){
        try{
            //Access Token Validation
            $user = $this->awsCognito->validateAccessToken();
            $udcObj = $this->me->getUdc($sysCode, $typeId);
            if($udcObj){
                ...
                $this->me->applyCriteria();
                $ds = $this->me->readAll();
                $dsHref = env("APP_URL").env("APP_VERSION") . "/systems/" . $sysCode . "/types/" . $typeId . "/values";
                if(isset($this->data['expand']) && $this->data['expand'] == 'type'){
                    $resultSet = $this->schema->renderOutputDS($this->schema, $ds, $dsHref, $recordSize, $offset, $limit, $user, "type", "TUDCTYPEFOUNDATION","", $udcObj);
                }else{
     ...
     */
    public function readOne($sysCode,$typeId,$valueId){
        try{
            //Access Token Validation
            $user = $this->awsCognito->validateAccessToken();
            $udcObj = $this->me->getUdc($sysCode, $typeId);
            if($udcObj){
                ...
                if($valueObj){
                    ...
                    $href = env('APP_URL') .env('APP_VERSION').'/systems/'. $sysCode . '/types/'. $typeId . "/values/" . $valueObj->_id;
                    $arrValue = $this->schema->renderOutput($valueObj, $href, $user);
                    if(isset($this->data['expand']) && $this->data['expand'] == 'type'){
                        $typeSchema = new SchemaRender("TUDCTYPEFOUNDATION");
                        $typeHref = env('APP_URL') .env('APP_VERSION').'/systems/'. $sysCode . '/types/'. $typeId;
