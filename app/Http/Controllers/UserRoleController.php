...
use App\Http\Controllers\ACL\RoleLibrary\UserRole;
...
class UserRoleController extends Controller{
    ...
    public function create($tenantId){
        try{
            ...
            if($validateSMS["status"]){
                ...
                //Checking for existing of user role resource for given name
                $criteria = new CriteriaOption();
                $roleObject = new UserRole($tenantId);
                $criteria->where(UserRole::FIELD_RECORD_TYPE, $roleObject->getRecordType());
                $criteria->where(UserRole::FIELD_ROLE_TYPE, $roleObject->getType());
                $criteria->where(UserRole::FIELD_TENANT, $roleObject->getTenantId());
                $criteria->where('name', $this->data['name']);
                $roleObject->pushCriteria($criteria);
                $roleObject->applyCriteria();
                $findObj = $roleObject->readOne();
                if(isset($findObj)){
                    //Validate request with unique name of user role
                ...
                //Render input and save new user role object
                ...
                $href = env('APP_URL') .env('APP_VERSION'). '/tenants/' . $tenantId . '/userroles/'. $outputObj->_id;
    ...
    /**
     * Updating a role resource of specific user by id/name
     * @date 11-Dec-2017
     * @author Setha Thay <setha.thay@workevolve.com>
     * @param  string $tenantId
     * @param  string $role
        ...
        try{
            ...
            if($validateSMS["status"]){
                ...
                if($updatedObj){
                    ...
                    $href = env('APP_URL') .env('APP_VERSION').'/tenants/'. $tenantId . '/userroles/'. $oldObj->_id;
                    ...
                    return ErrorHelper::getNotFoundError("User Role");
                }
            }else{
                return ErrorHelper::getExceptionError($validateSMS["message"]);
            }
    ...
    /**
     * Removing a role of a specific user resource by id/name
     * @date 11-Dec-2017
     * @author Setha Thay <setha.thay@workevolve.com>
     * @param  string $tenantId
     * @param  string $role
    ...
    public function remove($tenantId, $role){
        try{
            ...
            $user = $this->awsCognito->validateAccessToken();
            $criteria = new CriteriaOption();
            $criteria->where(UserRole::FIELD_RECORD_TYPE, $this->me->getRecordType());
            $criteria->where(UserRole::FIELD_ROLE_TYPE, $this->me->getType());
            $criteria->where(UserRole::FIELD_TENANT, $this->me->getTenantId());
            $criteria->whereRaw($this->me->getDefaultFilter($role),$this->schema->getAllFields(),$user);
            ...
            if($targetObj){
                ...
                if(!$relationshipObj->inUsed($targetObj, $this->schema->getPluralName())){
                    $href = env('APP_URL') .env('APP_VERSION').'/tenants/'. $tenantId . '/userroles/'. $targetObj->_id;
                    ...
                    return ErrorHelper::getInUsedError("User Role");
                }
            }else{
                return ErrorHelper::getNotFoundError("User Role");
            }
    ...
    public function readAll($tenantId){
        try{
            ...
            $this->me->pushCriteria($criteria);
            $this->me->applyCriteria();
            $dsHref = env("APP_URL").env("APP_VERSION") . "/tenants/" . $tenantId . "/userroles";
